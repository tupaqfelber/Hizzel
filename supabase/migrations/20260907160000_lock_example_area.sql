-- The onboarding example move ("Hizzel Now" -> "Hizzel New") should start
-- with its one floor locked by default, same as any floor a real user has
-- finished laying out and locked themselves -- it's a finished, staged demo
-- meant for exploring furniture placement, not editing the rooms
-- themselves. Only the `is_locked` value on the areas insert changes here;
-- everything else in seed_example_content() is unchanged from
-- 20260905170000_example_move_billing_exempt.sql.
