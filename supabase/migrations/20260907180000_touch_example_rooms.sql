-- The onboarding example move's two rooms had a 20cm gap between them
-- (Living Room spans canvas_x 0-400, Bedroom started at canvas_x 420) --
-- pointless now that the floor is locked by default (see
-- 20260907160000_lock_example_area.sql): a locked, fixed layout should
-- use all the space it has rather than leave an unexplained strip of
-- nothing between two rooms a new user can't even drag together
-- themselves. Bedroom now starts at canvas_x 400, flush against Living
-- Room's right edge. Only Bedroom's canvas_x changes from the previous
...
begin
  ...
  insert into areas (property_id, name, sort_order, is_locked)
  values (v_new_property_id, 'Ground floor', 0, true)
  ...
  insert into rooms (area_id, name, width_cm, depth_cm, canvas_x, canvas_y)
  values (v_area_id, 'Bedroom', 350, 300, 400, 0)
